...
Este script invoca ps_SyncAzureFiles.ps1 (modo orquestador con ventanas) para cada conjunto.
Editar variables en la sección Configuracion.
#>
...
Run-Sync `
  ...
  -LogFile (Join-Path $LogRoot "nas2-archivo-central\sync.log") `
  -PreservePermissions

# NAS1 BACKUP - DISCO PST
Ensure-Dir (Join-Path $LogRoot "nas1-hot\backup-disco-pst")
Run-Sync `
  ...
  -LogFile (Join-Path $LogRoot "nas1-hot\backup-disco-pst\sync.log") `
  -DoOnly "Correos PST MANUAL 2025" `
  -PreservePermissions

Ensure-Dir (Join-Path $LogRoot "nas1-cold\backup-disco-pst")
Run-Sync `
  -SourceRoot "\\Nasfvmed1\DISCOS EXTERNOS\BACKUP - DISCO PST" `
  -StorageAccount "storagefonvalmednas1" `
  -ShareName "nasfvmed1-cold" `
  -DestBaseSubPath "BACKUP - DISCO PST" `
  -LogFile (Join-Path $LogRoot "nas1-cold\backup-disco-pst\sync.log") `
  -DoOnly "DISCO PERSONAL" `
  -PreservePermissions

# NAS1 BACKUP WEB FONVALMED
Ensure-Dir (Join-Path $LogRoot "nas1-hot\backup-web-fonvalmed")
Run-Sync `
  -SourceRoot "\\Nasfvmed1\DISCOS EXTERNOS\BACKUP WEB FONVALMED" `
  -StorageAccount "storagefonvalmednas1" `
  -ShareName "nasfvmed1-hot" `
  -DestBaseSubPath "BACKUP WEB FONVALMED" `
  -LogFile (Join-Path $LogRoot "nas1-hot\backup-web-fonvalmed\sync.log") `
  -DoOnly "Backups Fonvalmed Web" `
  -PreservePermissions

# NAS1 DISCO1
Ensure-Dir (Join-Path $LogRoot "nas1-cold\disco1")
Run-Sync `
  -SourceRoot "\\Nasfvmed1\DISCOS EXTERNOS\DISCO1" `
  -StorageAccount "storagefonvalmednas1" `
  -ShareName "nasfvmed1-cold" `
  -DestBaseSubPath "DISCO1" `
  -LogFile (Join-Path $LogRoot "nas1-cold\disco1\sync.log") `
  -Exclude "BACKUP DANA PORTATIL" `
  -PreservePermissions

# NAS1 DISCO2 COLD
Ensure-Dir (Join-Path $LogRoot "nas1-cold\disco2")
Run-Sync `
  -SourceRoot "\\Nasfvmed1\DISCOS EXTERNOS\DISCO2" `
  -StorageAccount "storagefonvalmednas1" `
  -ShareName "nasfvmed1-cold" `
  -DestBaseSubPath "DISCO2" `
  -LogFile (Join-Path $LogRoot "nas1-cold\disco2\sync.log") `
  -DoOnly "1_4_1 PASAO;1_4_5 INFORMES MENSUALES;1_4_7 SST;02_AMB;CADENA;CONSORCIO LOS PARRA 2021;FONVALMED" `
  -PreservePermissions

# NAS1 DISCO2 COOL
Ensure-Dir (Join-Path $LogRoot "nas1-cool\disco2")
Run-Sync `
  -SourceRoot "\\Nasfvmed1\DISCOS EXTERNOS\DISCO2" `
  -StorageAccount "storagefonvalmednas1" `
  -ShareName "nasfvmed1-cool" `
  -DestBaseSubPath "DISCO2" `
  -LogFile (Join-Path $LogRoot "nas1-cool\disco2\sync.log") `
...
if ($correoNames -and $correoNames.Count -gt 0) {
  Ensure-Dir (Join-Path $LogRoot "nas1-cool\disco2-correos")
  ...
  Run-Sync `
    ...
    -ShareName "nasfvmed1-cool" `
    -DestBaseSubPath "DISCO2" `
    -LogFile (Join-Path $LogRoot "nas1-cool\disco2-correos\sync.log") `
    -DoOnly $correoDoOnly `
    -Exclude "INFO ANDRES GERALDO" `
    -PreservePermissions
} else {
...
# NAS1 DISCO3
Ensure-Dir (Join-Path $LogRoot "nas1-cold\disco3")
Run-Sync `
  -SourceRoot "\\Nasfvmed1\DISCOS EXTERNOS\DISCO3" `
  -StorageAccount "storagefonvalmednas1" `
  -ShareName "nasfvmed1-cold" `
  -DestBaseSubPath "DISCO3" `
  -LogFile (Join-Path $LogRoot "nas1-cold\disco3\sync.log") `
  -DoOnly "DISCO (C) JHON MORENO;DISCO BACKUP;INFO DISCO DURO PORTATIL" `
  -PreservePermissions

Ensure-Dir (Join-Path $LogRoot "nas1-cool\disco3")
Run-Sync `
  -SourceRoot "\\Nasfvmed1\DISCOS EXTERNOS\DISCO3" `
  -StorageAccount "storagefonvalmednas1" `
  -ShareName "nasfvmed1-cool" `
  -DestBaseSubPath "DISCO3" `
  -LogFile (Join-Path $LogRoot "nas1-cool\disco3\sync.log") `
  -DoOnly "YURLEY" `
  -PreservePermissions

# NAS1 HISTORIA DE LOS FUNCIONARIOS
Ensure-Dir (Join-Path $LogRoot "nas1-cold\historia-funcionarios")
Run-Sync `
  -SourceRoot "\\Nasfvmed1\DISCOS EXTERNOS\HISTORIA DE LOS FUNCIONARIOS" `
  -StorageAccount "storagefonvalmednas1" `
  -ShareName "nasfvmed1-cold" `
  -DestBaseSubPath "HISTORIA DE LOS FUNCIONARIOS" `
  -LogFile (Join-Path $LogRoot "nas1-cold\historia-funcionarios\sync.log") `
  -PreservePermissions

# NAS1 PST-CORREO CONTACTENOS
Ensure-Dir (Join-Path $LogRoot "nas1-cool\pst-correo-contactenos")
Run-Sync `
  -SourceRoot "\\Nasfvmed1\DISCOS EXTERNOS\PST-CORREO CONTACTENOS" `
  -StorageAccount "storagefonvalmednas1" `
  -ShareName "nasfvmed1-cool" `
  -DestBaseSubPath "PST-CORREO CONTACTENOS" `
  -LogFile (Join-Path $LogRoot "nas1-cool\pst-correo-contactenos\sync.log") `
  -DoOnly "CORREO CONTACTENOS 04-09-2023;CORREO CONTACTENOS 07-11-2024;CORREO CONTACTENOS 09-04-2024" `
  -PreservePermissions

Ensure-Dir (Join-Path $LogRoot "nas1-hot\pst-correo-contactenos")
Run-Sync `
  -SourceRoot "\\Nasfvmed1\DISCOS EXTERNOS\PST-CORREO CONTACTENOS" `
  -StorageAccount "storagefonvalmednas1" `
  -ShareName "nasfvmed1-hot" `
  -DestBaseSubPath "PST-CORREO CONTACTENOS" `
  -LogFile (Join-Path $LogRoot "nas1-hot\pst-correo-contactenos\sync.log") `
