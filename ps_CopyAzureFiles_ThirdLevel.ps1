...
if ([string]::IsNullOrWhiteSpace($logLeaf)) { $logLeaf = "azcopy-copy-third-level.log" }
...
if (-not $OpenNewWindows) {
  Write-Info "Modo directo: se copia toda la raiz."
  $code = Invoke-CopyWorker -WorkerSourcePath $srcRootResolved -WorkerDestSubPath $DestBaseSubPath -WorkerLogFile $LogFile
  exit $code
}
...
foreach ($item in $workItems) {
  ...
  $childLog   = if ($workLevel -eq "SecondLevel") {
    Join-Path $logParent ("copy-" + $safeSecond + ".log")
  } else {
    Join-Path $logParent ("copy-" + $safeSecond + "--" + $safeThird + ".log")
  }
...
"@
  ...
  if ($workLevel -eq "SecondLevel") {
    ...
    Write-Info ("Lanzado archivos_sueltos con copy: {0} -> {1}. ErrorLog='{2}'." -f $secondName, $destSub, $errorLogPath)
  } else {
