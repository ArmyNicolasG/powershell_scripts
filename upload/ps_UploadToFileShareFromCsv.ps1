...
# --- Validaciones ------------------------------------------------------------
if (!(Test-Path $SourceRoot)) { throw "SourceRoot no existe: $SourceRoot" }
$azcopy = (Get-Command azcopy.exe -ErrorAction SilentlyContinue)?.Source
if (-not $azcopy) { throw "AzCopy no está instalado/en PATH. Instálalo (v10+)." }
...
function Start-CsvSelectiveCopy {
  if (-not (Test-Path $CsvStructurePath)) { throw "CSV no encontrado: $CsvStructurePath" }

  Write-Host "[AzCopy] Cargando CSV de estructura: $CsvStructurePath"
  $rows = Import-Csv -Path $CsvStructurePath
  ...
  $allPaths = foreach ($r in $rows) {
    ...
    if ($p.StartsWith($SourceRoot, [StringComparison]::OrdinalIgnoreCase)) {
      ...
      # Si la fila no pertenece a la raíz, la ignoramos (o podrías lanzar error)
      # Write-Verbose "Fila fuera de raíz: $p"
    }
  }
